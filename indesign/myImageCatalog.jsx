		...
		with(borderPanels.add()){
			with(dialogColumns.add()){
				...
				with(dialogRows.add()){ 
					...
					var myHorizontalOffsetField = measurementEditboxes.add({editValue:0, editUnits:MeasurementUnits.points}); 
				} 
				with(dialogRows.add()){ 
					staticTexts.add({staticLabel:"Vertical Offset:", minWidth:myLabelWidth}); 
					var myVerticalOffsetField = measurementEditboxes.add({editValue:0, editUnits:MeasurementUnits.points}); 
				}
				with (dialogRows.add()){
					with(dialogColumns.add()){
...
function myAddLabel(myFrame, myLabelType, myLabelHeight, myLabelOffset, myLabelStyleName, myLayerName){
	...
	switch(myLabelType){
		...
		case 0:
            //var myre = /^.*?\.(.*)\.jpg/;
			myLabel = myLink.name;
			myLabel = myLabel.slice(0,-4);
			var labelG = myLabel.split(" ");
			tmp = labelG[0];
			labelG.shift();
			labelG.push(tmp);
			myLabel = labelG.join(" ");	
            //var myLabel2 = myre.exec(myLabel);
            //myLabel = myLabel[0];
			//var myTextFrame = myDocument.pages.item(0).textFrames.add();
			//myTextFrame.geometricBounds = ["6p", "6p", "24p", "24p"];
			//myTextFrame.contents = myLabel2[0];
			break;
		//File path
		case 1:
